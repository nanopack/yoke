...
func configureHBAConf() error {
	...
	for scanner.Scan() {
		...
		// if a special character is detected exit the scan immediately. This is done
		// to avoid duplicate comment blocks
...
`, conf.PGPort)
	...
	if master {
		entry += `
# added only to the node running postgres as 'master'
